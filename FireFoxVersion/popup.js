document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Load settings
    browserAPI.storage.local.get(["darkMode", "bassBoostEnabled"], (data) => {
        if (data.darkMode) {
    ...
    themeToggle.addEventListener("change", () => {
        ...
        browserAPI.storage.local.set({ darkMode: themeToggle.checked });
    });

    // Toggle bass boost visibility
    bassToggle.addEventListener("change", () => {
        bassBoostEnabled = bassToggle.checked;
        browserAPI.storage.local.set({ bassBoostEnabled: bassBoostEnabled });
    ...
    async function updatePopup() {
        ...
        const storedVolumes = await browserAPI.storage.local.get(null);
        ...
        tabs.forEach(tab => {
            ...
            marqueeContainer.onclick = () => browserAPI.tabs.update(tab.id, { active: true });
            ...
            volumeSlider.oninput = () => {
                ...
                browserAPI.storage.local.set({ [tab.id]: volumePercent });
        
                browserAPI.tabs.sendMessage(tab.id, { action: "updateVolume", tabId: tab.id, volume });
            };
            ...
            if (bassBoostEnabled) {
                ...
                bassSlider.oninput = () => {
                    ...
                    browserAPI.storage.local.set({ [`bass_${tab.id}`]: bassBoost });
                    
                    browserAPI.tabs.sendMessage(tab.id, { action: "updateBassBoost", tabId: tab.id, bassBoost });
                };
    ...
    browserAPI.tabs.onUpdated.addListener(updatePopup);
    browserAPI.tabs.onRemoved.addListener(updatePopup);
